...
If the system is connected to the Internet, the connected service (e.g. wireless network) is displayed:
...
If the system is not connected to the Internet a list of available services is displayed:
...
Network services that the system is connected to will additionally be marked in the list, whether or not they provide Internet access:

#+CAPTION: Connected network services are marked in list
#+NAME: fig:controller-network-service-marking
#+attr_html: :width 800px
[[../screenshots/controller-network-service-marking.png]]
