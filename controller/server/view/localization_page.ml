...
let select_form params options =
  form
    ~a:
      [ a_action params.action_url
      ; a_method `Post
      ; a_class [ "d-Localization__Form" ]
      ; Unsafe.string_attrib "is" "disable-after-submit"
      ]
    [ label ~a:[ a_class [ "d-Localization__Legend" ] ] [ txt params.legend ]
    ; select
        ~a:
          [ a_name params.select_name
    ...
    ; input
        ~a:[ a_input_type `Submit; a_class [ "d-Button" ]; a_value "Set" ]
        ()
    ]

let select_option current_id (id, name) =
  option
    ~a:([ a_value id ] @ if current_id = Some id then [ a_selected () ] else [])
    (txt name)

let timezone_form timezone_groups current_timezone =
  let timezone_group (group_id, timezones) =
    optgroup ~label:group_id
...
let html params =
  Page.html ~current_page:Page.Localization
    ~header:
      (Page.header_title ~icon:Icon.letter [ txt "Localization & Display" ])
    (div
       ...
       ; aside
           ...
           [ txt
               "Note that changes to the keyboard, language and display \
