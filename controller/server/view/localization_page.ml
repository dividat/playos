...
let generic_form params options =
  form
...
let generic_option current_id (id, name) =
  option
    ~a:(
      [ a_value id ]
...
let timezone_form timezone_groups current_timezone =
  let timezone_group (group_id, tzs) =
    optgroup ~label:group_id (List.map (generic_option current_timezone) tzs)
  in
  generic_form
...
let language_form langs current_lang =
  generic_form
    ...
    (List.map (generic_option current_lang) langs)
...
let keyboard_form keymaps current_keymap =
  generic_form
    ...
    (List.map (generic_option current_keymap) keymaps)
