...
let no_failure_gen () = Lwt.return false
...
let init_test_deps ?(failure_gen_rauc = no_failure_gen)
    ?(failure_gen_upd = no_failure_gen) ?(test_config = default_test_config) ()
    : test_context =
...
let product l1 l2 =
  List.concat_map (fun e1 -> List.map (fun e2 -> (e1, e2)) l2) l1
...
let all_possible_slot_spec_combos =
  let vsn_triples =
    product3 possible_versions possible_versions possible_versions
  in
  let combos =
    product3 vsn_triples possible_booted_slots possible_primary_slots
  in
  List.map
