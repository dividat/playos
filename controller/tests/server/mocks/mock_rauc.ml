...
class mock failure_generator =
  let return a =
    ...
    if should_fail then raise (Failure "Random test injected failure!")
    else Lwt.return a
  in
  object (self)
    val state : state =
    ...
    method private extract_version bundle_path =
      ...
      if m then Str.matched_group 1 bundle_path
      else
        Alcotest.fail @@ "Failed to extract version from bundle_path: "
        ^ bundle_path
