...
let daemon () =
  ...
  let peer =
    OBus_peer.make ~connection:system_bus ~name:"de.pengutronix.rauc"
  in
  return peer

let proxy daemon = OBus_proxy.make ~peer:daemon ~path:[]
...
let mark_slot daemon slot status =
  let%lwt marked, msg =
    OBus_method.call De_pengutronix_rauc_Installer.m_Mark (proxy daemon)
      (status, slot |> Slot.string_of_t)
  in
  let%lwt () = Logs_lwt.info ~src:log_src (fun m -> m "%s" msg) in
  if Slot.of_string marked = slot then return_unit
...
let slot_status_of_obus (o : (string * OBus_value.V.single) list) : Slot.status
    =
...
let install daemon source =
  ...
  let%lwt completed_e =
    OBus_signal.map
      ...
      (OBus_signal.make De_pengutronix_rauc_Installer.s_Completed proxy)
    |> OBus_signal.connect
  in
  let%lwt () =
